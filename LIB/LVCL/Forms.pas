...

{
  ...
  2010-04-13
    스크롤바 처리를 위해 WM_VSCROLL/WM_HSCROLL 메시지를 대상으로 전달.
    Application.ShowMainForm 처리.  
...
{$M-}
 ...
 private
   ...
   procedure SetShowMainForm(const Value: Boolean);
 public
   ...
   property ShowMainForm: Boolean read fShowMainForm write SetShowMainForm;
...
procedure TCustomForm.WMVScroll(var Message: TWMVScroll);
begin
  inherited;

  with Message do
  if IsWindow(ScrollBar) then
  with TMessage(Message) do
    SendMessage(ScrollBar, Msg, WParam, LParam);
end;

procedure TCustomForm.WMHScroll(var Message: TWMHScroll);
begin
  inherited;
...
procedure TApplication.SetShowMainForm(const Value: Boolean);
begin
  if Value = fShowMainForm then Exit;
  fShowMainForm := Value;

  if Assigned(fMainForm) then
  case Value of
    TRUE : fMainForm.Show;
    FALSE: fMainForm.Hide;  
  end;
end;
