...

{
  ...
  2010-03-30
    기존 TStream을 상속받은 객체들이 제대로 동작하지 않는 문제 수정을 위해
    GetSize, SetSize 의 인자값을 Cardinal -> LongInt 로 변경.

  2010-03-31
    TList 상속받을 때 Get/Put 이 GetItem/SetItem 으로 되어있는 문제.
    이 물건도 Get/Put 으로 변경.
    Capacity 를 설정하는 의미없는 변수도 추가.

  2010-04-13
    MakeObjectInstance 코드 추가.
...
type
  ...
  public
    Capacity: Integer; // TODO: 나중에 의미있게 바꿀 것!!
  end;
...
{$ifdef Linux} // this special function use stat() instead of seek()
    function GetSize: LongInt; override; {$endif}
...
procedure TList.Put(index: integer; value: pointer);
begin
  if (self=nil) or (cardinal(index)>=cardinal(fCount)) then
    exit;
  if fOwnObjects then
...
procedure TStream.SetSize(Value: LongInt);
begin
...
procedure TFileStream.SetSize(Value: LongInt);
begin
  Seek(Value, soFromBeginning);
{$ifdef Win32}
...
//   writeln(ClassName+'.'+Name+'='+Value);
   ...
   case ReadValueType of // no handler -> ignore this property
     ...
     raise EClassesError.Create('Unknown value type ' + IntToStr(Ord(ReadValueType)));
   end;
  {$endif}
  end; // with Reader do
end;
...
procedure TMemoryStream.SetSize(Value: LongInt);
begin
